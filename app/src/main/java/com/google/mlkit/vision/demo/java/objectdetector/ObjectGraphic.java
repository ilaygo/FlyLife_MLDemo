...
public class ObjectGraphic extends Graphic {
  ...
  public void draw(Canvas canvas) {
    ...
    float lineHeight = TEXT_SIZE + STROKE_WIDTH + 30;
    ...
    canvas.drawText("YDis: " + calculateYCordinates(rect) + " XDis: " + calculateXCordinates(rect) + " RealHeight: " + calculateRealHeight(rect) + "\n, width: " + rect.width() +  " , \n" + rect.toString() + "Tracking ID: " + object.getTrackingId() ,
        rect.left,
        rect.top + yLabelOffset,
        textPaints[colorID]);
    ...
    for (Label label : object.getLabels()) {
      if(label.getText() == "Home good" || label.getText() == "Fashion good" )
      {
        canvas.drawText("Landing Pad", rect.left, rect.top + yLabelOffset, textPaints[colorID]);
        yLabelOffset += lineHeight;
        canvas.drawText(
                String.format(Locale.US, LABEL_FORMAT, label.getConfidence() * 100, label.getIndex(), rect.toString()),
                rect.left,
                rect.top + yLabelOffset,
                textPaints[colorID]);
        Log.d("cen2", String.valueOf(rect.centerX()));
        Log.d("cen1", String.valueOf(rect.centerY()));
        yLabelOffset += lineHeight;
  ...
  public String calculateRealHeight(RectF rect12)
  {
    int TSize = 7;
    float TwidthPX = rect12.width();
    double miliRad = 0.31 * 2;
    double high =  7/TwidthPX/miliRad*1000;
    String formattedValue = String.format("%.2f", high);
    return formattedValue;
  }

  public String calculateYCordinates(RectF rect12)
  {
    float yCen = rect12.centerY();
    int yPicCen = 1080/2;
    double yDis = yPicCen - yCen;
    return pixelsToCentimers(yDis, rect12);
  }

  public String calculateXCordinates(RectF rect12)
  {
    float xCen = rect12.centerX();
    int xPicCen = 1920/2;
    double xDis = xPicCen - xCen;
    return pixelsToCentimers(xDis, rect12);

  }

  public String pixelsToCentimers(double pixel_distance, RectF rct)
  {
    double focal_length_cm = 0.24;
    double high = Double.parseDouble(calculateRealHeight(rct));
    double miliRad = 0.31 * 2;
    double size = miliRad*pixel_distance*high/1000;
    String formattedValue = String.format("%.2f", size);
    return formattedValue;
  }
}
